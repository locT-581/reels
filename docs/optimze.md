...
- Api/ directory có vẻ quá tải - có thể tách thành package riêng @xhub-reel/api-client

## `@xhub-reel/player-core` vs `@xhub-reel/player`
...
- `VideoFeedItem.tsx`: 
    + `feed` dùng `player`, `player` lại cần một số types từ `feed` hoặc `core`. Nếu không quản lý chặt, pnpm sẽ build rất chậm và runtime sẽ gặp lỗi undefined do thứ tự load module @xhub-reel/player-core được import trực tiếp NHƯNG đã có qua @xhub-reel/player, @xhub-reel/ui được import trực tiếp NHƯNG đã có qua @xhub-reel/player, Feed Package Over-Dependency, HIện tại: feed → [core, player, player-core, ui, gestures], đề xuất  feed → [core, player, gestures]
...
- Tôi cần xác nhận xem @xhub-reel/player có đang "re-export" quá nhiều từ player-core không . Nếu một dev muốn dùng UI của player nhưng lại phải gánh toàn bộ logic HLS của player-core khi họ chỉ muốn hiển thị một cái ảnh thumbnail, đó là lãng phí.
...
1. Contract-based Types: Tạo một package @xhub-reel/types riêng biệt. Cả core, player, feed đều import từ đây. Tránh việc feed import core chỉ để lấy một cái interface Video.
...
3. Headless Player Hook: @xhub-reel/player-core nên expose một hook (hoặc class) cực mạnh là useVideoEngine. Cái này không chứa UI, chỉ quản lý trạng thái máy trạng thái (FSM - Finite State Machine) của video.
